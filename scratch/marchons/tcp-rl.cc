...
TcpRl::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::TcpRl")
          ...
          .AddAttribute ("Reward", "Reward when increasing congestion window.", DoubleValue (1.0),
                         MakeDoubleAccessor (&TcpRl::m_reward), MakeDoubleChecker<double> ())
          .AddAttribute ("Penalty", "Reward when increasing congestion window.",
                         DoubleValue (-10.0), MakeDoubleAccessor (&TcpRl::m_penalty),
                         MakeDoubleChecker<double> ());
  return tid;
}

TcpRl::TcpRl (void) : TcpRlBase ()
{
  NS_LOG_FUNCTION (this);
}

TcpRl::TcpRl (const TcpRl &sock) : TcpRlBase (sock)
{
  NS_LOG_FUNCTION (this);
}
