...
{
  static TypeId tid = TypeId ("ns3::TcpGymEnv").SetParent<OpenGymEnv> ().SetGroupName ("OpenGym");
...
TcpEventGymEnv::GetObservation ()
{
  ...
  // 从自身的变量中找到m_socketUuid关于socket的id
...
// 不管是丢包还是接收到新的ACK，都会通知Agent，将当前的状态发送过去。
// 当丢包时调用这个函数
// 慢启动阈值
uint32_t
TcpEventGymEnv::GetSsThresh (Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
  ...
  NS_LOG_UNCOND ("getssthresh: " << num << " reward: " << m_envReward);

  // 这个函数动态改变了m_new_ssThresh的值,因为当发生丢包时，它告诉了Agent，然后Agent发来了新的动作，将内置变量m_new_ssThresh设置为了Action值，此时返回新值
  Notify ();
...
TcpEventGymEnv::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  NS_LOG_UNCOND(tcb->m_cWnd);
  NS_LOG_UNCOND(m_new_cWnd);
  tcb->m_cWnd = m_new_cWnd;
  
  NS_LOG_UNCOND ("increase window num, congesiton window, threshold and reward is "  << num << " " << tcb->m_cWnd << " " << tcb->m_ssThresh << " " << m_envReward);
}

// 这个函数调用的次数多于IncreaseWindow函数，每接收到一个ACK被调用一次。
void
TcpEventGymEnv::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time &rtt)
{
  ...
  num++;
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND (Simulator::Now () << " Node: " << m_nodeId << " PktsAcked, SegmentsAcked: "
  ...
  m_segmentsAcked = segmentsAcked;
  m_rtt = rtt;
  // NS_LOG_UNCOND("pktsacked " << num << " segment acked: " << segmentsAcked);
  NS_LOG_UNCOND ("pkts num is, congestion windows, threshold is " << num << " " << tcb->m_cWnd << " " << tcb->m_ssThresh);
}
...
TcpEventGymEnv::CwndEvent (Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
{
  ...
  NS_LOG_UNCOND (Simulator::Now ()
                 << " Node: " << m_nodeId << " CwndEvent: " << event << " " << eventName);
